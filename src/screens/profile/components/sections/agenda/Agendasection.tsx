...
const CalendarWidget: React.FC<{ days: CalendarDay[] }> = ({ days }) => {
  ...
  return (
    <View style={[cal.container, isDark && { backgroundColor: '#130d2a', borderColor: 'rgba(139,92,246,0.2)' }]}>
      <View style={cal.header}>
        <Text style={[cal.month, isDark && { color: '#f5f3ff' }]}>Febrero 2026</Text>
        <View style={cal.nav}>
          <TouchableOpacity style={[cal.navBtn, isDark && { backgroundColor: '#1a1030', borderColor: 'rgba(139,92,246,0.2)' }]}>
            <Text style={[cal.navTxt, isDark && { color: '#a78bfa' }]}>‹</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[cal.navBtn, isDark && { backgroundColor: '#1a1030', borderColor: 'rgba(139,92,246,0.2)' }]}>
            <Text style={[cal.navTxt, isDark && { color: '#a78bfa' }]}>›</Text>
          </TouchableOpacity>
      ...
      <View style={cal.labelsRow}>
        {DAY_LABELS.map(d => (
          <View key={d} style={cal.labelCell}>
            <Text style={[cal.labelTxt, isDark && { color: '#71717A' }]}>{d}</Text>
          </View>
        ))}
      </View>
      ...
      <View style={cal.legend}>
        {[
          ...
          <View key={i} style={cal.legendItem}>
            ...
            <Text style={[cal.legendTxt, isDark && { color: '#71717A' }]}>{item.label}</Text>
          </View>
        ))}
      </View>
    </View>
...
const ScheduleRow: React.FC<{ item: ScheduleItem }> = ({ item }) => {
  ...
  return (
    <View style={[sch.row, isDark && { backgroundColor: '#130d2a', borderColor: 'rgba(139,92,246,0.2)' }]}>
      <View style={[sch.dateBox, isDark && { backgroundColor: '#1a1030', borderColor: 'rgba(139,92,246,0.2)' }]}>
        <Text style={[sch.month, isDark && { color: '#71717A' }]}>{item.month}</Text>
        <Text style={[sch.day, isDark && { color: '#f5f3ff' }]}>{item.day}</Text>
      </View>
      <View style={sch.body}>
        <Text style={[sch.title, isDark && { color: '#f5f3ff' }]}>{item.title}</Text>
        <Text style={[sch.meta, isDark && { color: '#71717A' }]}>{item.time} · {item.location}</Text>
      </View>
      <View style={[sch.status, item.status === 'confirmed' ? sch.statusConfirmed : sch.statusPending]}>
        <Text style={[
          sch.statusTxt,
          item.status === 'confirmed' ? sch.statusTxtConfirmed : sch.statusTxtPending,
          item.status !== 'confirmed' && isDark && { color: '#fbbf24' },
        ]}>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.sectionHeader}>
        <Text style={[styles.label, isDark && { color: 'rgba(255,255,255,0.35)' }]}>DISPONIBILIDAD — FEBRERO 2026</Text>
        {isOwner && (
      ...
      <View style={styles.sectionHeader}>
        <Text style={[styles.label, isDark && { color: 'rgba(255,255,255,0.35)' }]}>PRÓXIMOS COMPROMISOS</Text>
        {isOwner && (
          <EditButton onPress={onEditSection || (() => {})} />
        )}
      </View>
