...
export const ArtistDetailView: React.FC<ArtistDetailViewProps> = ({
  ...
  // Efecto para simular o realizar el fetch
  useEffect(() => {
    if (open && artist) {
      setLoading(true);
      // NOTA: En Android, localhost es 10.0.2.2. Usa tu IP local para probar en dispositivo real.
      fetch(`http://192.168.1.XX:3001/api/v1/artists/${artist.id}`)
        .then((res) => res.json())
        .then((data) => setFullData(data))
        .catch((err) => {
          console.log('Error fetch (usando datos base):', err);
          // Si falla, no pasa nada, usamos los datos básicos
        })
