...
export const EditHeaderModal: React.FC<Props> = ({ visible, artist, onClose, onSave }) => {
  ...
  const [schedule, setSchedule] = useState('Lun a Vie 6-10pm');
  ...
  const [scheduleDays, setScheduleDays] = useState({
    lunes: { enabled: true, start: '6:00pm', end: '10:00pm' },
    martes: { enabled: true, start: '6:00pm', end: '10:00pm' },
    miercoles: { enabled: true, start: '6:00pm', end: '10:00pm' },
    jueves: { enabled: true, start: '6:00pm', end: '10:00pm' },
    viernes: { enabled: true, start: '6:00pm', end: '10:00pm' },
  ...
  useEffect(() => {
    if (visible) {
      ...
      setSchedule(artist.schedule ?? 'Lun a Vie 6-10pm');
      setBio(artist.bio ?? '');
  ...
  const handleSave = () => {
    // Convertir el horario del selector a formato de texto
    const enabledDays = Object.entries(scheduleDays)
    ...
    if (enabledDays.length > 0) {
      ...
      // Simple formato: si todos los días de lunes a viernes tienen el mismo horario
      const weekdays = enabledDays.filter(d => ['lunes', 'martes', 'miercoles', 'jueves', 'viernes'].includes(d.name));
      if (weekdays.length === 5 && weekdays.every(d => d.start === weekdays[0].start && d.end === weekdays[0].end)) {
        scheduleText = `Lun-Vie ${weekdays[0].start.replace(':00', '')}-${weekdays[0].end.replace(':00', '')}`;
      } else {
        // Formato individual para cada día
        scheduleText = enabledDays.map(d => `${dayNames[d.name]} ${d.start.replace(':00', '')}-${d.end.replace(':00', '')}`).join(', ');
      }
    } else {
