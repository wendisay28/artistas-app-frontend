...
import ReviewCard from '../../explore/shared/ReviewCard';
import { Colors } from '../../../theme';
import InfoPair from '../../explore/shared/InfoPair';
...
const EmptyItem = ({ label1, label2, label3, icon }: { label1: string, label2: string, label3: string, icon: any }) => (
  <View style={vs.emptyItemRow}>
    <View style={vs.iconCircle}>
      <Ionicons name={icon} size={20} color={Colors.text3 ?? '#aaa'} />
    </View>
      ...
      <View style={sr.row}>
        {socials.map((item) => {
          ...
          return (
            ...
            >
              <Ionicons name={item.icon} size={22} color={active ? '#fff' : (Colors.text3 ?? '#aaa')} />
