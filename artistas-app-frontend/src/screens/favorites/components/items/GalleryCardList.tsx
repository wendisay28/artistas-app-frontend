// GalleryCardList.tsx — Card de lista para galería en Favoritos (v2)
...
export const GalleryCardList: React.FC<GalleryCardListProps> = ({
  item,
  onPress,
  onToggleFavorite,
}) => {
  const [saved, setSaved] = useState(true);
  const isAvail = item.availability === 'Disponible';
    ...
    >
      ...
      <View style={listCardStyles.imageContainer}>
        ...
        <View style={listCardStyles.ratingBadge}>
          ...
          <Text style={listCardStyles.ratingText}>{item.rating}</Text>
        </View>
      ...
      <View style={listCardStyles.info}>
        <View style={listCardStyles.infoTop}>
          ...
          <View style={listCardStyles.nameRow}>
            <Text style={listCardStyles.name} numberOfLines={1}>{item.name}</Text>
          </View>

          {/* Artista */}
          <Text style={listCardStyles.subtitle} numberOfLines={1}>{item.artistName}</Text>

          {/* Ubicación */}
          <View style={listCardStyles.metaRow}>
            <Ionicons name="location-outline" size={11} color={colors.textSecondary} />
            <Text style={listCardStyles.metaText} numberOfLines={1}>{item.location}</Text>
          </View>

          {/* Técnica */}
          <View style={listCardStyles.metaRow}>
            <Ionicons name="color-palette-outline" size={11} color={colors.textSecondary} />
            <Text style={listCardStyles.metaText} numberOfLines={1}>{item.medium}</Text>
          </View>
        </View>
        ...
        <View style={listCardStyles.infoBottom}>
          <View>
            <Text style={listCardStyles.priceLabel}>Precio</Text>
            <View style={listCardStyles.priceRow}>
              <Text style={listCardStyles.price}>{formatPrice(item.price)}</Text>
            </View>
          </View>
