// EventCardList.tsx — Card de lista para eventos en Favoritos (v2)
      ...
      <View style={listCardStyles.info}>
        <View style={listCardStyles.infoTop}>
          ...
          <View style={listCardStyles.nameRow}>
            <Text style={listCardStyles.name} numberOfLines={1}>{event.name}</Text>
          </View>

          {/* Fecha · hora */}
          <Text style={listCardStyles.subtitle} numberOfLines={1}>{event.date} · {event.time}</Text>

          {/* Ubicación */}
          <View style={listCardStyles.metaRow}>
            <Ionicons name="location-outline" size={11} color={colors.textSecondary} />
            <Text style={listCardStyles.metaText} numberOfLines={1}>{event.location}</Text>
          </View>

          {/* Venue */}
          <View style={listCardStyles.metaRow}>
            <Ionicons name="business-outline" size={11} color={colors.textSecondary} />
            <Text style={listCardStyles.metaText} numberOfLines={1}>{event.venue}</Text>
          </View>
        </View>
        ...
        <View style={listCardStyles.infoBottom}>
          <View>
            <Text style={listCardStyles.priceLabel}>Entrada</Text>
            <View style={listCardStyles.priceRow}>
              <Text style={listCardStyles.price}>{formatPrice(event.price)}</Text>
            </View>
          </View>
