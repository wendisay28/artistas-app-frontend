...
import { ARTIST_CATEGORIES, getDisciplinesByCategory, getRolesByDiscipline } from '../../../../constants/artistCategories';
...
export default function FilterModal({
  ...
  const setArtistCategory = (categoryId: string) => {
    haptic();
    setDraft((d: ExploreFilters) => ({ 
      ...d, 
      artistCategoryId: categoryId,
      artistDisciplineId: undefined,
      artistRoleId: undefined,
    }));
  };

  const setArtistDiscipline = (disciplineId: string) => {
    haptic();
    setDraft((d: ExploreFilters) => ({ 
      ...d, 
      artistDisciplineId: disciplineId,
      artistRoleId: undefined,
    }));
  };

  const setArtistRole = (roleId: string) => {
    haptic();
    setDraft((d: ExploreFilters) => ({ ...d, artistRoleId: roleId }));
  };
  ...
  const activeFiltersCount = [
    ...
    draft.artistCategoryId !== undefined,
    draft.artistDisciplineId !== undefined,
    draft.artistRoleId !== undefined,
    ...
    >
      ...
      >
        ...
        >
          ...
          <View style={styles.section}>
            ...
            <View style={styles.togglesCol}>
              ...
              >
                ...
                <Toggle active={draft.nearMe ?? false} />
              </Pressable>
              ...
              >
                ...
                <Toggle active={draft.availableToday ?? false} />
              </Pressable>
          ...
          <View style={styles.section}>
            <Text style={styles.sectionLabel}>Categorías de Artistas</Text>
            
            {/* Categoría */}
            <Text style={styles.subLabel}>Categoría</Text>
            <View style={styles.chipsRow}>
              {ARTIST_CATEGORIES.map((category) => {
                const isActive = draft.artistCategoryId === category.id;
                return (
                  <Pressable
                    key={category.id}
                    onPress={() => setArtistCategory(category.id)}
                    style={({ pressed }) => [
                      styles.chip,
            ...
            {/* Disciplina */}
            {draft.artistCategoryId && (
              <>
                <Text style={styles.subLabel}>Disciplina</Text>
                <View style={styles.chipsRow}>
                  {getDisciplinesByCategory(draft.artistCategoryId).map((discipline) => {
                    const isActive = draft.artistDisciplineId === discipline.id;
                    return (
                      <Pressable
                        key={discipline.id}
                        onPress={() => setArtistDiscipline(discipline.id)}
                        style={({ pressed }) => [
                          styles.chip,
                          isActive && styles.chipActive,
                          pressed && { opacity: 0.75 },
            ...
            {/* Rol */}
            {draft.artistCategoryId && draft.artistDisciplineId && (
              <>
                <Text style={styles.subLabel}>Rol</Text>
                <View style={styles.chipsRow}>
                  {getRolesByDiscipline(draft.artistCategoryId, draft.artistDisciplineId).map((role) => {
                    const isActive = draft.artistRoleId === role.id;
                    return (
                      <Pressable
                        key={role.id}
                        onPress={() => setArtistRole(role.id)}
                        style={({ pressed }) => [
                          styles.chip,
                          isActive && styles.chipActive,
                          pressed && { opacity: 0.75 },
