        ...
        <View style={styles.card}>
          ...
          <View style={styles.socialRow}>
            {[
              ...
              { label: 'X', icon: 'logo-x', color: '#000000', hasData: !!socialLinks?.x },
              ...
              <TouchableOpacity
                key={item.label}
                style={[styles.socialIcon, item.hasData ? { backgroundColor: item.color } : styles.socialInactive]}
                onPress={async () => {
                  ...
                  if (item.hasData) {
                    ...
                    } else if (item.label === 'X' && socialLinks?.x) {
                      url = `https://twitter.com/${socialLinks.x}`;
                    ...
                    if (Platform.OS !== 'web') Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                    // TODO: Implementar modal de edición de redes sociales
                  }
                }}
              >
                <View style={[styles.iconWrap, item.hasData ? { backgroundColor: item.color } : styles.socialInactive]}>
                  <Ionicons name={item.icon as any} size={22} color={item.hasData ? '#fff' : (Colors.text3 ?? '#aaa')} />
                  <Text style={[styles.label, !item.hasData && styles.labelInactive]}>{item.label}</Text>
                </View>
              </TouchableOpacity>
            ))}
          </View>
